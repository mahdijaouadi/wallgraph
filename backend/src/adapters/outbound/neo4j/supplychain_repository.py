from backend.src.domain.ports import SupplyChainRepository
from backend.src.domain.models import Ticker, TickerSupplierRelationship
import requests
...
from backend.src.adapters.outbound.neo4j.common_queries import create_node, add_relation, delete_node
from backend.src.adapters.outbound.agents.supplier_relationship.workflow import Workflow
from backend.src.adapters.outbound.embedding_model import GeminiEmbedding3072Async
...
class Neo4jSupplyChainRepository(SupplyChainRepository):
    def __init__(self, tx: AsyncTransaction) -> None:
        ...
        self.tx = tx
        self.embeding_model_deduplication= GeminiEmbedding3072Async()
    ...
    async def get_secfiling(self,ticker: Ticker)-> str:
        ...
        for company in companies.values():
            if company['ticker'] == ticker.ticker_name.upper():
    ...
    async def check_filing_existence(self, ticker:  Ticker, sec_filing_date: str) -> bool:
        ...
        """
        result = await self.tx.run(query, {"ticker_name": ticker.ticker_name, "filing_date":sec_filing_date})        
    ...
    async def delete_supply_chains(self,ticker: Ticker) -> str:
        ...
        DELETE rel
        """
        result = await self.tx.run(query, {"ticker_name": ticker.ticker_name})
    ...
    async def supplier_deduplication(self) -> str:
        ...
        companies_embeddings = await self.embeding_model_deduplication.embed_documents(company_names)

        country_names = [s["supplier_name"] for s in supplier_countries]
        countries_embeddings = await self.embeding_model_deduplication.embed_documents(country_names)

        region_names = [s["supplier_name"] for s in supplier_regions]
        regions_embeddings = await self.embeding_model_deduplication.embed_documents(region_names) 
